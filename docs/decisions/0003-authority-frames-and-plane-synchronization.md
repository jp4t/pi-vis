...
- A semantic frame contains ordered records, one complete terminal semantic snapshot, and a typed child-authored runtime-resume checkpoint. Main routes the semantic payload opaquely and the renderer reduces it atomically. Main may retain the resume checkpoint separately only as an owner-local continuation token and return it unchanged to a replacement host for the same session record; it must not derive one from compatibility state or apply one across records. The child uses it only when persisted active-branch model/thinking metadata is absent.
...
- Complete post-navigation branches are child-owned presentation escrow, not repeated semantic snapshot or journal state. The live terminal record is a fast path; attach repeats every unacknowledged branch. Navigation captures escrow before releasing custody drain, and later semantic mutation/replacement stays fenced until the renderer atomically installs the newest matching branch and sends an exact owner/intent acknowledgement. Tree-overlay lifetime does not own or cancel this reconciliation.
